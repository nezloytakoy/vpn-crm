...
export async function POST(request: Request) {
    try {
        ...
        if (assistantRequests === currentAssistantRequests) {
            ...
            // 1) Берём шаблон из переводов
            let increasedMessage = getTranslation(userLang, "requests_increased");
            // 2) Заменяем плейсхолдер "%count%" на фактическое число diff
            increasedMessage = increasedMessage.replace("%count%", diff.toString());
